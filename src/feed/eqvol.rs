...
pub async fn eqvol() -> Result<(), Box<dyn std::error::Error>> {
...
fn load() -> Result<String, Box<dyn std::error::Error>> {
    let content = fs::read_to_string("./data/eqvol.xml").expect("could not read file");
    Ok(content)
}

fn save(xml: String) -> Result<(), Box<dyn std::error::Error>> {
    let mut file = File::create("./data/eqvol.xml")?;
    writeln!(file, "{}", xml)?;
    file.flush()?;
    Ok(())
}
