...
pub async fn extra() -> Result<(), Box<dyn std::error::Error>> {
    let resp = reqwest::get("https://www.data.jma.go.jp/developer/xml/feed/extra.xml")
        .await?
        .text()
        .await?;
...
fn load() -> Result<String, Box<dyn std::error::Error>> {
    let content = fs::read_to_string("./data/extra.xml").expect("could not read file");
    Ok(content)
}

fn save(xml: String) -> Result<(), Box<dyn std::error::Error>> {
    let mut file = File::create("./data/extra.xml")?;
    writeln!(file, "{}", xml)?;
    file.flush()?;
    Ok(())
}
